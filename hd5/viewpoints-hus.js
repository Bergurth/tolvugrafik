...
function render()
{
	...
	case 8:
	    ...
	    break;
	case 9:
	    // Static viewpoint from -40, 140; camera follows car
	    mv = lookAt( vec3(-40.0, 140.0, 15.0+height), vec3(carXPos, carYPos, 0.0), vec3(0.0, 0.0, 1.0 ) );
	    drawScenery( mv );
	    mv = mult( mv, translate(carXPos, carYPos, 0.0) );
	    mv = mult( mv, rotateZ( -carDirection ) ) ;
	    drawCar( mv );
