...
var hall_length = 15000

// Hnútar veggsins
var vertices = [
    vec4( - hall_length,  0.0, 0.5, 1.0 ),
    vec4(  hall_length,  0.0, 0.5, 1.0 ),
    vec4(  hall_length,  1.0, 0.5, 1.0 ),
    vec4(  hall_length,  1.0, 0.5, 1.0 ),
    vec4( - hall_length,  1.0, 0.5, 1.0 ),
    vec4( - hall_length,  0.0, 0.5, 1.0 ),
//
    vec4( - hall_length,  0.0, 2.5, 1.0 ),
    vec4(  hall_length,  0.0, 2.5, 1.0 ),
    vec4(  hall_length,  1.0, 2.5, 1.0 ),
    vec4(  hall_length,  1.0, 2.5, 1.0 ),
    vec4( - hall_length,  1.0, 2.5, 1.0 ),
    vec4( - hall_length,  0.0, 2.5, 1.0 ),
...
// Hnútar gólfsins (strax á eftir)
    vec4( - hall_length,  0.0, 10.0, 1.0 ),
    vec4(  hall_length,  0.0, 10.0, 1.0 ),
    vec4(  hall_length,  0.0,  0.0, 1.0 ),
    vec4(  hall_length,  0.0,  0.0, 1.0 ),
    vec4( - hall_length,  0.0,  0.0, 1.0 ),
    vec4( - hall_length,  0.0, 10.0, 1.0 ),
/*
    // vidbaetur
    vec4( -5.0,  0.0, 1.0, 1.0 ),
    vec4(  5.0,  0.0, 1.0, 1.0 ),
...
var texCoords = [
    vec2(  0.0, 0.0 ),
    vec2( 1.33 * hall_length, 0.0 ),
    vec2( 1.33 * hall_length, 2.0 ),
    vec2( 1.33 * hall_length, 2.0 ),
    vec2(  0.0, 2.0 ),
    vec2(  0.0, 0.0 ),
...
// Mynsturhnit fyrir gólf
    ...
    vec2(  0.0,  0.0 ),
    vec2( 0.66 * hall_length,  0.0 ),
    vec2( 0.66 * hall_length, 10.0 ),
    vec2( 0.66 * hall_length, 10.0 ),
    vec2(  0.0, 10.0 ),
    vec2(  0.0,  0.0 ),
    // vidbod
...
window.onload = function init() {
    ...
    var veggImage = document.getElementById("RockImage");
    ...
    var golfImage = document.getElementById("RockImage");
    ...
    //var veggImage = document.getElementById("GolfImage");
    texVegg2 = gl.createTexture();
    gl.bindTexture( gl.TEXTURE_2D, texVegg2 );
    gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL, true);
    gl.texImage2D( gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, veggImage2 );
    gl.generateMipmap( gl.TEXTURE_2D );
    gl.texParameteri( gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR_MIPMAP_LINEAR );
    gl.texParameteri( gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.LINEAR );
...
var render = function(){
    ...
    gl.bindTexture( gl.TEXTURE_2D, texVegg2 );
    gl.drawArrays( gl.TRIANGLES, 2 * numVertices, 3 * numVertices );
