...
var materialDiffuse = vec4(
    parseInt(colorInt.toString()[2]),
    parseInt(colorInt.toString()[1]),
    parseInt(colorInt.toString()[0]),
    1.0 );
...
window.onload = function init() {
    ...
    discardValueLoc = gl.getUniformLocation( program, "discardValue" );
	...
	case 37: // left arrow
	    ...
	    var colIntStr = colorInt.toString();
	    if(colIntStr.length < 3){
		colIntStr = '0'.repeat(3 - colIntStr.length) + colIntStr;
	    }
	    materialDiffuse = vec4(
		parseInt(colIntStr[0]),
		parseInt(colIntStr[1]),
		parseInt(colIntStr[2]),
		1.0 );
	    diffuseProduct = mult(lightDiffuse, materialDiffuse);
	    gl.uniform4fv( gl.getUniformLocation(program, "diffuseProduct"), flatten(diffuseProduct) );
	    render();
